...
-- -----------------------------------------------------
--        Constraints, Keys & Data Integrity          --
-- -----------------------------------------------------
...
create table status_table (id INT primary key, status VARCHAR(20) default "Active");

insert into status_table (id) values (1);

insert into employees (employee_id, employee_name) values (1, "Test"); -- Duplicate ID gives primary key error if 1 already exist

insert into employees (employee_id, employee_name, email) values (999, "Test User", "alice@gmail.com"); -- Duplicate email giv
...
insert into departments values (1, "Data Science");

insert into employees values (101, "Test Employee", 1);

insert into employees
values (102, "Wrong Department", 999); -- foreign key error.
...
select employee_name,department_name ,employee_city from employees as e
left join departments as d
on e.department_id = d.department_id
...
select avg(salary),department_name from employees as e
inner join departments as d
on e.department_id = d.department_id
group by department_name;
...
select sum(salary),department_name from departments as d
left join employees as e
on e.department_id = d.department_id
group by d.department_name;
...
select department_name,count(project_name) as count from departments as d
inner join projects as p
on d.department_id = p.department_id
group by department_name
having count >= 1;
...
select m.employee_name as manager ,count(e.employee_name) from employees as e
inner join employees as m
on e.manager_id=m.employee_id
group by manager;
...
inner join employees as m
on e.manager_id=m.employee_id
group by m.employee_name,m.employee_id
having count(e.employee_name) > 3;

select m.employee_name as employee,e.employee_name as manager,m.salary from employees as e
inner join employees as m
on e.employee_id = m.manager_id;
...
select d.department_name,sum(p.budget),d.budget from departments as d
inner join projects as p
on d.department_id = p.department_id
...
having avg(salary) = (
	select  max(avg_salary) from(
            ...
            group by department) as debt_avg
            );
...
select * from employees
where department in (
select department from employees where employee_city = "hyderabad");
...
where exists (
select departments from employees as e
where d.department_id = e.department_id);
